...
"Refinements agreed during implementation" (dom_count_min / dom_count_frac). Levels A/B/C
replace v1's dom_now/dom_20d/dom_50d naming (today / persistent-over-a-trailing-window),
per SM's design; only the ONE table a row's own `level` needs is computed.
"""
...
def dom_count_threshold(group_size: int, dom_count_min: int, dom_count_frac: float) -> int:
    """threshold = max(dom_count_min, ceil(dom_count_frac * group_size)). dom_count_frac=0
    reproduces strategy_grp v1's fixed absolute count exactly (used for parity runs)."""
...
def group_dominance_now(groups: pd.Series, dominance_attribute: str, dominance_direction: bool,
                        dominance_decile: float, dom_count_min: int, dom_count_frac: float
...
def resolve_dom_table(groups: pd.Series, params: dict) -> tuple[pd.DataFrame, pd.Series]:
    ...
    dom_now, cutoffs = group_dominance_now(
        ...
        params["dominance_decile"], params["dom_count_min"], params["dom_count_frac"],
    )
    level = params["level"]
    if level == "A":
        return dom_now, cutoffs
