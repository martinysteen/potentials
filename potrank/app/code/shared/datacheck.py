...
Copied verbatim from strategy_grp2/app/code/shared/datacheck.py (which itself was copied
verbatim from strategy_grp v1) — this module is deliberately generic, knows nothing about
potrank's column list, and must not be edited to add project-specific behaviour. The list
of files a run actually needs is assembled by potrank's own preflight.py, which is where
the project-specific knowledge lives; this module takes that list as an argument.

Why this exists
---------------
`repositoryRTBI/data/` is not a static input — it is rewritten all day long by independent
cron jobs that are closely timed but not synchronised:
...
def _diagnosis(verdict: Verdict, root: Path) -> str:
    ...
    lines += [
        "",
        "repositoryRTBI is rewritten on a cron all day (rclone sync :07/:37/:55, longi :15),",
        "so this is usually transient — re-run a few minutes later, or:",
