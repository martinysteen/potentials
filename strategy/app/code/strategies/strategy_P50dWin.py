"""
Strategy: P50dWin
Select tickers where longi_P50d_win >= 0.9 at the test daynum,
...
def find_start_daynum(gain20_df: pd.DataFrame, rank_df: pd.DataFrame,
    ...
    for col in gain20_df.columns:
        ...
        if has_rank and has_gain and has_p50dw:
            return daynum
    raise ValueError("No valid starting daynum found — check future_gain20d.csv, longi_rank.csv, and longi_P50d_win.csv")
...
def main() -> None:
    ...
    start_daynum = find_start_daynum(gain_df, rank_df, p50d_win_df)
    min_daynum = max(
    ...
    print(f"Focusset size: {n}   Step: {step}   Period: {period}d   P50d_win >= {PARAMS['p50d_win_min']}")
    print()
    ...
    while daynum >= min_daynum:
        tickers = select_focusset(daynum, p50d_win_df, rank_df, n)
        if not tickers:
            daynum -= step
            continue
    ...
    print()
    if not hop_results:
        print("No valid hops produced — no tickers met P50d_win >= threshold in the data range")
        sys.exit(1)
...
def build_extension():
    ...
    return run_extension(STRATEGY_NAME, PARAMS,
                         lambda d: select_focusset(d, p50d_win_df, rank_df, n),
