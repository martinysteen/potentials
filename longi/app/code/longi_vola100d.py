"""
100-day Volatility Module (Returns-based)

Calculates volatility as standard deviation of daily returns over a 100-day rolling window.
Returns are calculated as percentage changes: (price[t] - price[t-1]) / price[t-1] * 100

Volatility = stdev(daily_returns_over_100_days)

Output is in percentage points (e.g., 2.5 means 2.5% daily volatility).

Reads PotDat.csv and outputs longi_vola100d.csv with identical structure.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_vola100d.csv"
WINDOW_SIZE = 100
...
def calculate_vola_100d(prices: List[float]) -> List[Optional[float]]:
    """
    Calculate 100-day volatility as standard deviation of daily returns.

    Array layout: [0]=newest (2009), [n-1]=oldest (1543)
    For day at index i, we need prices at indices [i:i+100] to calculate 100 returns
    (100 prices give 99 returns, but we include price[i+100] to get 100 returns)
    ...
    """
    n = len(prices)
    if n < WINDOW_SIZE + 1:  # Need 101 prices to calculate 100 returns
        return [None] * n

    vola_values = []

    for i in range(n):
        # Need 101 prices to calculate 100 returns
        window_end = min(i + WINDOW_SIZE + 1, n)
        window = prices[i:window_end]

        # Need full window (101 prices for 100 returns)
        if len(window) < WINDOW_SIZE + 1:
            vola_values.append(None)
            continue

        # Calculate 100 daily returns
        returns = []
...
def process_ticker_row(ticker: str, price_strings: List[str]) -> List[Optional[float]]:
    """
    Process a single ticker row: parse prices and calculate 100-day volatility.
    ...
    vola_values = calculate_vola_100d(prices)
...
def main() -> int:
    ...
    try:
        ...
        print(f"3. Calculating 100-day volatility for {len(rows)} tickers...")
        for row in rows:
            ticker = row[0]
            price_strings = row[1:]
