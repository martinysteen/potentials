"""
CoreIndex Price per Ticker Module

For each ticker, looks up its CoreIndex (market index) from Stamdata.csv,
then copies that index's full price row from PotDat.csv.
Result is a standard longi_*.csv file (rows=tickers, columns=daynums).

Depends on: nothing (reads PotDat.csv and Stamdata.csv)
Outputs: longi_coreindex.csv
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_coreindex.csv"
...
def main() -> int:
    ...
    """
    print("CoreIndex price per ticker")

    # Check input files
    for f in [STAMDATA_FILE, POTDAT_FILE]:
        if not f.exists():
    ...
    try:
        ...
        # 2. Load PotDat: header + all rows, build index ticker → values lookup
        print(f"2. Reading prices from {POTDAT_FILE.name}")
        ...
        print(f"   {len(tickers)} tickers, {num_daynums} daynums")
        ...
        print(f"   {len(index_data)} index tickers found in PotDat")

        # Empty row for tickers with no match
        empty_values = [''] * num_daynums

        # 3. Build output: for each ticker, copy its index's price row
        print(f"3. Expanding index prices to individual tickers...")
        output_rows = []
        matched = 0
        unmatched = 0
        ...
        for ticker in tickers:
            ...
            if idx_ticker and idx_ticker in index_data:
                output_rows.append([ticker] + index_data[idx_ticker])
                matched += 1
            else:
