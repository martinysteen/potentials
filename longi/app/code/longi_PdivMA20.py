"""
Price / MA20 Ratio Module

Calculates the ratio of current price to 20-day moving average, expressed as percentage.

Formula: P/MA20 = (current_price / ma20) * 100

Values > 100: price above MA20 (bullish)
Values < 100: price below MA20 (bearish)

Depends on: longi_ma20.py (needs longi_ma20.csv)
Reads PotDat.csv and longi_ma20.csv, outputs longi_PdivMA20.csv with identical structure.
...
INPUT_MA_FILE = Path(__file__).parent.parent / "output" / "longi_ma20.csv"
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_PdivMA20.csv"
...
def calculate_price_ma_ratio(prices: List[Optional[float]], ma_values: List[Optional[float]]) -> List[Optional[float]]:
    """
    Calculate ratio of price to MA20, expressed as percentage.

    Args:
        prices: List of prices
        ma_values: List of MA20 values
...
def main() -> int:
    ...
    if not INPUT_MA_FILE.exists():
        print(f"ERROR: MA20 file not found: {INPUT_MA_FILE}")
        print(f"       Run longi_ma20.py first to generate this file")
        return 1
    ...
    try:
        ...
        # Read MA20 CSV
        print(f"2. Reading MA20 file: {INPUT_MA_FILE.name}")
        ...
        print(f"4. Calculating P/MA20 ratios for {len(price_rows)} tickers...")
