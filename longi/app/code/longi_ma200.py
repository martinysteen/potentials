"""
200-day Simple Moving Average Module

Calculates 200-day SMA (Simple Moving Average) of stock prices.

Formula: SMA = sum(prices_over_200_days) / 200

Reads PotDat.csv and outputs longi_ma200.csv with identical structure.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_ma200.csv"
WINDOW_SIZE = 200
...
def calculate_ma200(prices: List[float]) -> List[Optional[float]]:
    """
    Calculate 200-day simple moving average.

    Array layout: [0]=newest (2009), [n-1]=oldest (1543)
    For day at index i, the 200-day window includes [i:i+200] (current day + 199 preceding days)

    Args:
        prices: List of prices from newest to oldest (left to right in CSV)

    Returns:
        List of MA200 values (same length as prices, None where calculation not possible)
    """
    n = len(prices)
    if n < WINDOW_SIZE:
        return [None] * n
    ...
    for i in range(n):
        # Get 200-day window: current day + 199 preceding days
...
def process_ticker_row(ticker: str, price_strings: List[str]) -> List[Optional[float]]:
    """
    Process a single ticker row: parse prices and calculate 200-day MA.
    ...
    Returns:
        List of MA200 values (None where calculation not possible)
    """
    ...
    ma_values = calculate_ma200(prices)
...
def main() -> int:
    ...
    """
    print(f"=== longi_ma200.py START: 200-day simple moving average calculation ===")
    ...
    try:
        ...
        print(f"3. Calculating 200-day MA for {len(rows)} tickers...")
        for row in rows:
            ticker = row[0]
            price_strings = row[1:]
