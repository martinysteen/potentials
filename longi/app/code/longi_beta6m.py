...
Calculates beta (market sensitivity) over a 6-month period (133 trading days).

Beta = Cov(stock_returns, index_returns) / Var(index_returns)

Where:
- stock_returns = daily returns of the stock over 133 days
- index_returns = daily returns of the stock's core index over the same 133 days
...
Reads PotDat.csv and Stamdata.csv, outputs longi_beta6m.csv.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_beta6m.csv"
...
def main() -> int:
    print(f"6-month beta calculation (Cov/Var over {WINDOW_SIZE} days)")
    ...
    print(f"4. Calculating 6-month beta for {len(rows)} tickers...")
