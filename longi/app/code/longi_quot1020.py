"""
MA10 / MA20 Quotient Module (speed indicator)

Calculates the ratio of the 10-day to the 20-day moving average, expressed as
percentage. Measures short-term momentum acceleration ("Cross1020 as a
quotient"): the faster MA pulling away above the slower one.

Formula: quot1020 = (ma10 / ma20) * 100

Values > 100: MA10 above MA20 (accelerating / bullish)
Values < 100: MA10 below MA20 (decelerating / bearish)

Depends on: longi_ma10.py and longi_ma20.py (needs longi_ma10.csv, longi_ma20.csv)
Reads both MA files, outputs longi_quot1020.csv with identical structure.
...
# Configuration
INPUT_MA_FAST_FILE = Path(__file__).parent.parent / "output" / "longi_ma10.csv"
INPUT_MA_SLOW_FILE = Path(__file__).parent.parent / "output" / "longi_ma20.csv"
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_quot1020.csv"
LABEL_FAST = "MA10"
LABEL_SLOW = "MA20"
