"""
Price / MA200 Ratio Module

Calculates the ratio of current price to 200-day moving average, expressed as percentage.

Formula: P/MA200 = (current_price / ma200) * 100

Values > 100: price above MA200 (bullish)
Values < 100: price below MA200 (bearish)

Depends on: longi_ma200.py (needs longi_ma200.csv)
Reads PotDat.csv and longi_ma200.csv, outputs longi_PdivMA200.csv with identical structure.
...
INPUT_MA_FILE = Path(__file__).parent.parent / "output" / "longi_ma200.csv"
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_PdivMA200.csv"
...
def calculate_price_ma_ratio(prices: List[Optional[float]], ma_values: List[Optional[float]]) -> List[Optional[float]]:
    """
    Calculate ratio of price to MA200, expressed as percentage.

    Args:
        prices: List of prices
        ma_values: List of MA200 values
...
def main() -> int:
    ...
    """
    print(f"=== longi_PdivMA200.py START: Price / MA200 ratio calculation ===")
    ...
    if not INPUT_MA_FILE.exists():
        print(f"ERROR: MA200 file not found: {INPUT_MA_FILE}")
        print(f"       Run longi_ma200.py first to generate this file")
        return 1
    ...
    try:
        ...
        # Read MA200 CSV
        print(f"2. Reading MA200 file: {INPUT_MA_FILE.name}")
        ...
        print(f"4. Calculating P/MA200 ratios for {len(price_rows)} tickers...")
