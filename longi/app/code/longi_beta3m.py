...
Calculates beta (market sensitivity) over a 3-month period (67 trading days).

Beta = Cov(stock_returns, index_returns) / Var(index_returns)

Where:
- stock_returns = daily returns of the stock over 67 days
- index_returns = daily returns of the stock's core index over the same 67 days
...
Reads PotDat.csv and Stamdata.csv, outputs longi_beta3m.csv.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_beta3m.csv"
...
def main() -> int:
    print(f"3-month beta calculation (Cov/Var over {WINDOW_SIZE} days)")
    ...
    print(f"4. Calculating 3-month beta for {len(rows)} tickers...")
