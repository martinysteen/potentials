...
Calculates beta (market sensitivity) over a 1-year period (265 trading days).

Beta = Cov(stock_returns, index_returns) / Var(index_returns)

Where:
- stock_returns = daily returns of the stock over 265 days
- index_returns = daily returns of the stock's core index over the same 265 days
...
Reads PotDat.csv and Stamdata.csv, outputs longi_beta1yr.csv.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_beta1yr.csv"
...
def main() -> int:
    print(f"1-year beta calculation (Cov/Var over {WINDOW_SIZE} days)")
    ...
    print(f"4. Calculating 1-year beta for {len(rows)} tickers...")
