"""
Sector2 1-Year Sector Performance per Ticker Module

Expands sector-aggregated 1-year performance (from longi_grp_Sector2_1yr.csv) back to
individual ticker level using the Sector2 mapping from Stamdata.csv.

For each ticker, looks up its Sector2, then copies that sector's full row of values.
Result is a standard longi_*.csv file (rows=tickers, columns=daynums).

Depends on: longi_grp_Sector2_1yr.py (needs longi_grp_Sector2_1yr.csv)
Reads Stamdata.csv and longi_grp_Sector2_1yr.csv, outputs longi_Sector2_1yr.csv.
...
GRP_FILE = Path(__file__).parent.parent / "output_grp" / "longi_grp_Sector2_1yr.csv"
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_Sector2_1yr.csv"
...
def load_grp_data() -> tuple[List[str], Dict[str, List[str]]]:
    """
    Load longi_grp_Sector2_1yr.csv.
...
def main() -> int:
    ...
    """
    print("Sector2 1-year sector performance per ticker")
    ...
    try:
        ...
        # Empty row for tickers with no Sector2 match
