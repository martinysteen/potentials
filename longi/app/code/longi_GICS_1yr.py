"""
GICS 1-Year Sector Performance per Ticker Module

Expands sector-aggregated 1-year performance (from longi_grp_GICS_1yr.csv) back to
individual ticker level using the GICS mapping from Stamdata.csv.

For each ticker, looks up its GICS sector, then copies that sector's full row of values.
Result is a standard longi_*.csv file (rows=tickers, columns=daynums).

Depends on: longi_grp_GICS_1yr.py (needs longi_grp_GICS_1yr.csv)
Reads Stamdata.csv and longi_grp_GICS_1yr.csv, outputs longi_GICS1yr.csv.
...
GRP_FILE = Path(__file__).parent.parent / "output_grp" / "longi_grp_GICS_1yr.csv"
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_GICS_1yr.csv"


def load_ticker_to_gics() -> Dict[str, str]:
    """Load ticker → GICS mapping from Stamdata.csv (column index 5)."""
    mapping = {}
...
def load_grp_data() -> tuple[List[str], Dict[str, List[str]]]:
    """
    Load longi_grp_GICS_1yr.csv.

    Returns:
        Tuple of (daynums_header, sector_data)
        - daynums_header: header row as list of strings (timestamp + daynums)
        - sector_data: dict mapping GICS sector name → list of values (one per daynum)
    """
...
def main() -> int:
    ...
    """
    print("GICS 1-year sector performance per ticker")
    ...
    try:
        # 1. Load GICS mapping
        print(f"1. Reading GICS mapping from {STAMDATA_FILE.name}")
        ticker_to_gics = load_ticker_to_gics()
        print(f"   {len(ticker_to_gics)} ticker→GICS mappings loaded")

        # 2. Load sector-aggregated data
        ...
        # 3. Load ticker list from PotDat
        print(f"3. Reading ticker list from {POTDAT_FILE.name}")
        tickers = load_ticker_list()
        print(f"   {len(tickers)} tickers")

        # Empty row for tickers with no GICS match
        empty_values = [''] * num_daynums

        # 4. Build output: for each ticker, copy its sector's row
        ...
        for ticker in tickers:
            ...
            if gics and gics in sector_data:
                output_rows.append([ticker] + sector_data[gics])
                matched += 1
            else:
