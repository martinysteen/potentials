"""
1-Year Sharpe Ratio Module

Calculates Sharpe ratio over a 1-year period (265 trading days = 12×22+1).

Sharpe = Period Return / Period Volatility

Where:
- Period Return = (price_today - price_265_days_ago) / price_265_days_ago * 100 (percentage)
- Period Volatility = stdev(daily_returns_over_265_days) (percentage points)

Output is dimensionless ratio (return/volatility).

Reads PotDat.csv and outputs longi_sh1yr.csv with identical structure.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_sh1yr.csv"
WINDOW_SIZE = 265  # 1 year = 12 × 22 + 1 trading days
...
def calculate_sharpe_1yr(prices: List[float]) -> List[Optional[float]]:
    """
    Calculate 1-year Sharpe ratio (return / volatility).
...
def process_ticker_row(ticker: str, price_strings: List[str]) -> List[Optional[float]]:
    """
    Process a single ticker row: parse prices and calculate 1-year Sharpe ratio.
    ...
    sharpe_values = calculate_sharpe_1yr(prices)
...
def main() -> int:
    ...
    """
    print(f"1-year Sharpe ratio calculation (return/volatility)")
    ...
    try:
        ...
        print(f"3. Calculating 1-year Sharpe ratio for {len(rows)} tickers...")
        for row in rows:
            ticker = row[0]
            price_strings = row[1:]
