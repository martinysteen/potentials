"""
Spread to 100-day Maximum Module

Calculates the percentage growth needed to reach the maximum stock price
seen in the last 100 days (rolling 100-day window).

Formula: spread = ((max_100d - current_price) / current_price) * 100

Reads PotDat.csv and outputs longi_spr100d.csv with identical structure.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_spr100d.csv"
WINDOW_SIZE = 100
...
def calculate_spread_100d(prices: List[float]) -> List[Optional[float]]:
    """
    Calculate percentage spread to 100-day maximum.

    Array layout: [0]=newest (2009), [n-1]=oldest (1543)
    For day at index i, the 100-day window includes [i:i+100] (current day + 99 preceding days)

    Args:
        prices: List of prices from newest to oldest (left to right in CSV)

    Returns:
        List of spread values (same length as prices, None where calculation not possible)
    """
    n = len(prices)
    if n < WINDOW_SIZE:
        return [None] * n
    ...
    for i in range(n):
        # Get 100-day window: current day + 99 preceding days
        ...
        if current_price > 0:
            spread = ((max_price - current_price) / current_price) * 100
...
def process_ticker_row(ticker: str, price_strings: List[str]) -> List[Optional[float]]:
    """
    Process a single ticker row: parse prices and calculate spread to 100-day max.
    ...
    Returns:
        List of spread values (None where calculation not possible)
    """
    ...
    # Calculate spread for valid prices
    spread_values = calculate_spread_100d(prices)

    # Pad with None for the empty values we didn't process
    remaining = len(price_strings) - len(prices)
    spread_values.extend([None] * remaining)
...
def main() -> int:
    ...
    """
    print(f"=== longi_spr100d.py START: Spread to 100-day maximum calculation ===")
    ...
    try:
        ...
        print(f"3. Calculating spread to 100-day max for {len(rows)} tickers...")
        for row in rows:
            ticker = row[0]
            price_strings = row[1:]

            # Calculate spread
            spread_values = process_ticker_row(ticker, price_strings)

            # Format output row
            spread_strings = [format_european_decimal(val) for val in spread_values]
            output_row = [ticker] + spread_strings
            output_rows.append(output_row)

            # Count processing stats
            if any(val is not None for val in spread_values):
                tickers_processed += 1
            else:
                tickers_skipped += 1
        ...
        print(f"ERROR: Error during spread calculation: {e}")
        return 1
