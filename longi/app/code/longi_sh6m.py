"""
6-Month Sharpe Ratio Module

Calculates Sharpe ratio over a 6-month period (133 trading days = 6×22+1).

Sharpe = Period Return / Period Volatility

Where:
- Period Return = (price_today - price_133_days_ago) / price_133_days_ago * 100 (percentage)
- Period Volatility = stdev(daily_returns_over_133_days) (percentage points)

Output is dimensionless ratio (return/volatility).

Reads PotDat.csv and outputs longi_sh6m.csv with identical structure.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_sh6m.csv"
WINDOW_SIZE = 133  # 6 months = 6 × 22 + 1 trading days
...
def calculate_sharpe_6m(prices: List[float]) -> List[Optional[float]]:
    """
    Calculate 6-month Sharpe ratio (return / volatility).
...
def process_ticker_row(ticker: str, price_strings: List[str]) -> List[Optional[float]]:
    """
    Process a single ticker row: parse prices and calculate 6-month Sharpe ratio.
    ...
    sharpe_values = calculate_sharpe_6m(prices)
...
def main() -> int:
    ...
    """
    print(f"6-month Sharpe ratio calculation (return/volatility)")
    ...
    try:
        ...
        print(f"3. Calculating 6-month Sharpe ratio for {len(rows)} tickers...")
        for row in rows:
            ticker = row[0]
            price_strings = row[1:]
