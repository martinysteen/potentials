# Longi Project - Context for Gemini Code
...
- **longi_upload.py** - Upload results to Google Drive ✓ IMPLEMENTED
  - Syncs output/ and across/ directories
...
**across_<daynum>.csv** - Cross-sectional view for most recent daynum ✓ IMPLEMENTED
    ...
    - Generated by longi.py calling aux_across.make_across() (runs last, depends on all other modules)
    - Uses maximum (newest) daynum from PotDat.csv
    - **Manual historical generation**: Use aux_across.make_across(daynum, target_folder) to create backfilled files for specific daynums
      - Example: `python3 -c "from aux_across import make_across; make_across(1950, './historical/')"`
...
- **Output structure**:
  ...
  - Example columns: ticker_2009, rsi, macd_line, macd_signal, macd_histogram, uptrend, per1d, per1w, per1m, per3m, per6m, per1y, rank, median_10d, median_20d, median_30d, median_40d, median_50d, median_100d, stepup, **GICS_1yr, Sector2_1yr**
...
- ✓ Pipeline orchestrator (longi.py) fully implemented
  ...
  - 21 modules registered: rsi, macd, macd_Z, uptrend, performance, rank, medians, stepup, spr100d, spr250d, vola20d, vola100d, ma20, ma50, ma200, PdivMA20, PdivMA50, PdivMA200, grp_GICS_1yr, grp_Sector2_1yr, across
...
- ✓ longi_grp_Sector2_1yr.py fully implemented
  - Outputs: output_grp/longi_grp_Sector2_1yr.csv (Sector2-aggregated 1-year growth)
  - Aggregates individual stock growth by Sector2 attribute (56 sectors)
  - Formula: average(1 + growth_rate) - 1
- ✓ longi_across.py & aux_across.py fully implemented
  - aux_across.make_across(daynum, target_folder) function for programmatic use
...
- Gemini Code shortcut: Ctrl+Alt+G
