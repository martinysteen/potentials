#!/usr/bin/env python3
"""
Regression Analysis: Which factors predict future gains?
Analyzes spr100d, spr250d, vola20d, vola100d as predictors of future_gain20d and future_gain50d.
"""
...
X_FILES = {
    "spr100d": OUTPUT_DIR / "longi_spr100d.csv",
    "spr250d": OUTPUT_DIR / "longi_spr250d.csv",
    "vola20d": OUTPUT_DIR / "longi_vola20d.csv",
    "vola100d": OUTPUT_DIR / "longi_vola100d.csv",
}
...
def load_longi_csv(filepath: Path) -> pd.DataFrame:
    ...
    # First row might be a timestamp - skip if not a ticker
    if df.index[0] and 'GMT' in str(df.columns[0]):
        # Header row contains timestamp, skip it
        pass
    return df
...
def main():
    ...
    print(data[list(x_dfs.keys()) + list(y_dfs.keys())].describe().round(4))
    ...
    for y_col in y_dfs.keys():
        result = run_regression(data, x_cols, y_col)
    ...
    print("=" * 70)
    print("""
- R-squared: How much variance in future gains is explained by these factors
- Standardized coefficients (Beta): Relative importance of each predictor
  - Positive beta: Higher values predict higher future gains
  - Negative beta: Higher values predict lower future gains
- Statistical significance: * p<0.05, ** p<0.01, *** p<0.001

Key questions answered:
1. Which factors have the strongest predictive power? (highest |Beta|)
2. Do the same factors matter for 20d vs 50d gains?
3. Are spread or volatility measures more predictive?
""")
