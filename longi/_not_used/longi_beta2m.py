...
Calculates beta (market sensitivity) over a 2-month period (45 trading days).

Beta = Cov(stock_returns, index_returns) / Var(index_returns)

Where:
- stock_returns = daily returns of the stock over 45 days
- index_returns = daily returns of the stock's core index over the same 45 days
...
Reads PotDat.csv and Stamdata.csv, outputs longi_beta2m.csv.
...
OUTPUT_FILE = Path(__file__).parent.parent / "output" / "longi_beta2m.csv"
WINDOW_SIZE = 45  # 2 months = 2 × 22 + 1 trading days
...
def main() -> int:
    print(f"2-month beta calculation (Cov/Var over {WINDOW_SIZE} days)")
    ...
    print(f"4. Calculating 2-month beta for {len(rows)} tickers...")
